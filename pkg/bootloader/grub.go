...
func (g *Grub) installEFIPartitionBinaries(rootDir, efiDir, efiPath, deviceLabel string) error {
	...
	if efiPath == constants.FallbackEFIPath {
		...
		case constants.ArchAmd64, constants.Archx86:
			shimImg = filepath.Join(installPath, constants.EfiImgX86)
		case constants.ArchArm64:
			shimImg = filepath.Join(installPath, constants.EfiImgArm64)
		default:
...
// ListPersistentVariables reads key value pairs from the given persistent variables file
...
func (g *Grub) Install(rootDir, bootDir, stateLabel string) (err error) {
	...
	if !g.disableBootEntry {
		err = g.DoEFIEntries(filepath.Base(g.shimImg), constants.EfiDir)
		if err != nil {
			return err
		}
	}
