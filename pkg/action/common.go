...
func applySelinuxLabels(cfg *v1.Config, parts v1.ElementalPartitions) error {
	...
	if mnt, _ := utils.IsMounted(cfg, parts.Persistent); mnt {
		binds[parts.Persistent.MountPoint] = constants.UsrLocalPath
	}
	if mnt, _ := utils.IsMounted(cfg, parts.OEM); mnt {
		binds[parts.OEM.MountPoint] = constants.OEMPath
	}
	return utils.ChrootedCallback(
		cfg, constants.WorkingImgDir, binds, func() error { return elemental.SelinuxRelabel(cfg, "/", true) },
	)
}
